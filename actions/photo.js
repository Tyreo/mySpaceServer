...
module.exports = {
  ...
  async getPhotosByType(type, pageIndex, pageSize) {
    ...
    switch (type) {
      ...
      case "accepted":
        [count, photos] = await Promise.all([
          photo.getApprovedPhotosCount(),
          photo.getApprovedPhotos(pageIndex, pageSize),
        ]);
        return {
          count,
          data: photos,
      ...
      case "rejected":
        [count, photos] = await Promise.all([
          photo.getUnApprovedPhotosCount(),
          photo.getUnApprovedPhotos(pageIndex, pageSize),
        ]);
        return {
          count,
          data: photos,
      ...
      default:
        [count, photos] = await Promise.all([
          photo.getAllCount(),
          photo.getAll(pageIndex, pageSize),
        ]);
        return {
          count,
          data: photos,
  ...
  async getAlbums(userId, pageIndex, pageSize) {
    let albums, count;
    if (pageSize) {
      [albums, count] = await Promise.all([
        album.getAlbumsCount(userId),
        album.getAlbums(userId, pageIndex, pageSize),
      ]);
    } else {
      albums = await album.getAlbums(userId);
    }
    let result = await Promise.all(
      albums.map(async function (item) {
        const id = item._id;
