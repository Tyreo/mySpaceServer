...
router.get("/login", async (context, next) => {
  ...
  const data = await account.login(code);
  context.body = {
    status: 0,
    data: data,
  };
...
router.get("/test", async (context, next) => {
  context.logger.info(`test接口开始调用`);
  const code = context.query.code;
  context.body = {
    status: 0,
    data: {
...
router.put(
  ...
  async (context, next) => {
    context.logger.info(
      ...
      }, 修改的内容为${JSON.stringify(context.request.body)}`
    );
...
router.get("/album", auth, async (context, next) => {
  ...
  const albums = await photo.getAlbums(
    context.state.user.id,
 ...
 * 小程序种获取相册列表
 */
router.get("/syreo/album", auth, async (context, next) => {
 ...
 * 小程序种获取相册的相片列表
 */
router.get("/syreo/album/:id", auth, async (context, next) => {
  const photos = await photo.getPhotos(
    context.state.user.id,
    context.params.id
  );
...
router.post(
  ...
  async (context, next) => {
    ...
    await photo.add(
      context.state.user.id,
      `https://static.syreo.cn/${file.filename}`,
      id
    );
    await next();
  },
 ...
 * 上传头像
 */
router.post("/avatar", auth, uplader.single("file"), async (context, next) => {
  const { file } = context.req;
  const url = `https://static.syreo.cn/${file.filename}`;
  await account.uploadAvatar(context.state.user.id, url);
  context.body = {
    status: 0,
    data: {
      url,
...
router.get("/admin/photo/:type", auth, async (context, next) => {
  ...
  const photos = await photo.getPhotosByType(
    context.params.type,
    pageParams.pageIndex,
    pageParams.pageSize
  );
...
router.get("/admin/user/:type", async (context, next) => {
  ...
  context.body = {
    status: 0,
    data: await account.getUsersByType(
      context.params.type,
      pageParams.pageIndex,
      pageParams.pageSize
    ),
